...
export class Roulette {
    ...
    public rollByIndex(index: number) {
        ...
        const point = 360 * (index)
            / (this.sections.length) + 360
        ...
        const ival = setInterval(() => {
            const next_rotation = - sprint
                ...
                * (milliseconds - 2) - this.board.shift
    ...
    private drawSVGRoulette() {
        ...
        this.sections.map((section, index) => {
            ...
            const tx = radius + (radius-radius/3) * Math.sin(angle * (index + 0.5)) + this.board.padding;
            const ty = radius + (radius-radius/3) * -Math.cos(angle * (index + 0.5)) + this.board.padding;
            const translate = 'translate('+ tx +','+ ty +')';
            const rotate = 'rotate(' + degree + ')';
            const text = section.text ?? section.index.toString();
